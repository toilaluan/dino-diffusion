...
class ELLA(nn.Module):
    ...
    def forward(self, text_encode_features, attention_mask, timesteps):
        B = text_encode_features.size(0)
        device = text_encode_features.device
        dtype = text_encode_features.dtype
        all_outputs = []
        for i in range(B):
            seqlen = attention_mask[i].sum().item()
            x = text_encode_features[i, :seqlen, :].unsqueeze(0)
            t = timesteps[i].unsqueeze(0)
            ori_time_feature = self.position(t.view(-1)).to(device, dtype=dtype)
            ori_time_feature = (
                ori_time_feature.unsqueeze(dim=1)
                if ori_time_feature.ndim == 2
                else ori_time_feature
            )
            ori_time_feature = ori_time_feature.expand(len(x), -1, -1)
            time_embedding = self.time_embedding(ori_time_feature)

            encoder_hidden_states = self.connector(
                x, timestep_embedding=time_embedding
            )
            all_outputs.append(encoder_hidden_states)
